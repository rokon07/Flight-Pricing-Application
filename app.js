...
function depart(){
    ...
    fetch(`https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/autosuggest/v1.0/UK/GBP/en-GB/?query=${document.getElementById("userCity").value}`, {
        "method": "GET",
        "headers": {
            "x-rapidapi-host": "skyscanner-skyscanner-flight-search-v1.p.rapidapi.com", //API used is Skyscanner from RapidApi
            "x-rapidapi-key": "29678f958bmsh6b6d7d13ca2ec08p1797a0jsn9c08d7d1c764" //application unique key
        }
    })
    ...
    .then(data => { //use the data from get request
        ...
        while (i < data.Places.length ){ //go over all the returned data and put them in the options list
            var opt = document.createElement('option'); //make option element to fill it with data of airport
            opt.value = data.Places[i].PlaceId;
            opt.innerHTML = data.Places[i].PlaceName + " ( " + data.Places[i].PlaceId + ")"; //fill options list for user to choose from
...
function getCurrency() {
    ...
    fetch("https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/reference/v1.0/currencies", {
        ...
        "headers": {
            "x-rapidapi-host": "skyscanner-skyscanner-flight-search-v1.p.rapidapi.com", //API used is Skyscanner from RapidApi
            "x-rapidapi-key": "a6606f3be9mshea8930f7ecb575cp1e6bd7jsn536225f3ce3c" //application unique key
        }
    })
    .then(response => response.json()) //get response in json
    .then(data => { //use the data from get request
...
function arrive() {
    //API Get request: gets list of airports
    fetch(`https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/autosuggest/v1.0/UK/USD/en-GB/?query=${document.getElementById("userCityArrive").value}`, {
        "method": "GET",
        "headers": {
            "x-rapidapi-host": "skyscanner-skyscanner-flight-search-v1.p.rapidapi.com", //API used is Skyscanner from RapidApi
            "x-rapidapi-key": "29678f958bmsh6b6d7d13ca2ec08p1797a0jsn9c08d7d1c764" //application unique key
...
function getResult(){
    ...
    fetch(`https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/browseroutes/v1.0/US/${currencyType}/en-US/${arriveAirport}/${depAirport}/${getDep}?inboundpartialdate=${getArrive}`, {
        "method": "GET",
        "headers": {
            "x-rapidapi-host": "skyscanner-skyscanner-flight-search-v1.p.rapidapi.com", //API used is Skyscanner from RapidApi
            "x-rapidapi-key": "29678f958bmsh6b6d7d13ca2ec08p1797a0jsn9c08d7d1c764" //application unique key
    ...
    .then(data => { //use the data from get request
        ...
        var count = 0;
        var j;
        for(j = 0; j < data.Quotes.length; j++) { //go over all the quotes
